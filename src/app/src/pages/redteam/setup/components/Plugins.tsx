...
export default function Plugins({ onNext, onBack }: PluginsProps) {
  ...
  const renderPluginCategory = (category: string, plugins: readonly Plugin[]) => {
    ...
    return (
      ...
      >
        <AccordionSummary expandIcon={<ExpandMoreIcon sx={{ color: '#fff' }} />} sx={{ backgroundColor: '#2B1449', borderRadius: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
            <Typography variant="h6" sx={{ fontWeight: 'medium', flex: 1, color: '#fff' }}>
            ...
            {isExpanded && (
              ...
              >
                <Box component="span" onClick={() => { pluginsToShow.forEach((plugin) => { if (!selectedPlugins.has(plugin)) { handlePluginToggle(plugin); } }); }}>Select all</Box>
                <Box component="span" onClick={() => { pluginsToShow.forEach((plugin) => { if (selectedPlugins.has(plugin)) { handlePluginToggle(plugin); } }); }}>Select none</Box>
              </Box>
            )}
          </Box>
        </AccordionSummary>
        <AccordionDetails sx={{ backgroundColor: '#2B1449', borderRadius: 2 }}>
              ...
              <Grid item xs={12} sm={6} md={4} key={plugin}>
                <Paper
                  ...
                  sx={{
                    p: 2,
                    height: '100%',
                    border: selectedPlugins.has(plugin) ? `1px solid #A259F7` : '1px solid #444',
                    backgroundColor: selectedPlugins.has(plugin) ? '#2B1449' : '#22103B',
                    color: '#fff',
                    borderRadius: 2,
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
                      ...
                      label={
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                          {category === 'Recently Used' && getPluginCategory(plugin) && (
                            <Typography variant="caption" sx={{ backgroundColor: '#A259F7', px: 1, py: 0.25, borderRadius: 1, color: '#fff', alignSelf: 'flex-start' }}>
                    ...
                    {selectedPlugins.has(plugin) && PLUGINS_SUPPORTING_CONFIG.includes(plugin) && (
                      <IconButton
                        ...
                        onClick={(e) => { e.stopPropagation(); handleConfigClick(plugin); }}
                        sx={{
                          position: 'absolute',
                          top: 8,
                          right: 8,
                          ...
                          ...(PLUGINS_REQUIRING_CONFIG.includes(plugin) && !isPluginConfigured(plugin) && { color: 'error.main', opacity: 1 }),
                        }}
                      >
    ...
    <ErrorBoundary FallbackComponent={ErrorFallback}>
      <Box>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          >
            <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ backgroundColor: '#2B1449', borderRadius: 2 }}>
              <Typography variant="h6" sx={{ fontWeight: 'medium', color: '#fff' }}>
                Custom Prompts (
        ...
        >
          <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ backgroundColor: '#2B1449', borderTopLeftRadius:2,borderRadius:2, borderTopRightRadius:2,borderBottomLeftRadius:0,borderBottomRightRadius:0 }}>
            <Typography variant="h6" sx={{ fontWeight: 'medium', color: '#fff' }}>
              Custom Policies (
          ...
          <AccordionDetails sx={{ backgroundColor: '#2B1449',borderTopLeftRadius:0, borderTopRightRadius:0,borderBottomLeftRadius:2,borderBottomRightRadius:2 }}>
